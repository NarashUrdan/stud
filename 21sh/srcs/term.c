...
/*   Updated: 2018/04/24 16:02:06 by slegros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_term_check(char **env)
{
	...
	{
		ft_putendl_col_fd("21sh: TERM not set in the environnement", RED, 2);
		free(env_term);
		exit(EXIT_FAILURE);
	}
	if (tgetent(NULL, env_term) < 1)
	{
		ft_putendl_col_fd("21sh: failed to load terminfo database", RED, 2);
		free(env_term);
		exit(EXIT_FAILURE);
	}
	free(env_term);
...
{
	ft_putstr_col_fd("21sh: failed to ", RED, 2);
