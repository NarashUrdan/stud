...
/*   Updated: 2018/05/06 17:27:35 by slegros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_unsetenv_error(char *str, int mode)
{
	if (mode == 7 || mode == 8)
	{
		if (mode == 8)
		{
			ft_putstr_col_fd("21sh: unsetenv: Illegal option -- ", YEL, 2);
			ft_putchar_col_fd(str[0], YEL, 2);
			ft_putchar_col_fd('\n', YEL, 2);
		}
		else
			ft_putendl_col_fd("21sh: unsetenv: Not enough arguments", YEL, 2);
		ft_putendl_col_fd("usage: unsetenv [name]...", YEL, 2);
	}
	else if (mode == 9)
	{
		ft_putstr_col_fd("21sh: unsetenv: Incorrect identifer: ", YEL, 2);
...
static void		ft_setenv_error(char *str, int mode)
{
	if (mode == 2 || mode == 3)
	{
		if (mode == 3)
		{
			ft_putstr_col_fd("21sh: setenv: Illegal option -- ", YEL, 2);
			ft_putchar_col_fd(str[0], YEL, 2);
			ft_putchar_col_fd('\n', YEL, 2);
		}
		else
			ft_putendl_col_fd("21sh: setenv: Not enough arguments", YEL, 2);
		ft_putendl_col_fd("usage: setenv [name=value]...", YEL, 2);
	}
	else if (mode == 6)
	{
		ft_putstr_col_fd("21sh: setenv: Bad argument: ", YEL, 2);
		ft_putendl_col_fd(str, YEL, 2);
	}
	else
...
int				ft_env_error(char *str, int mode)
{
	...
	if (mode == 1)
	{
		...
		while (str[i] == 'i')
			i++;
		ft_putstr_col_fd("21sh: env: Illegal option -- ", YEL, 2);
	...
	else if (mode == 5)
	{
		...
		ft_putendl_col_fd(": Undefined results", YEL, 2);
	}
	else
