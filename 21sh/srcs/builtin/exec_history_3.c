...
/*   Updated: 2018/05/07 19:02:55 by slegros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_putstr_col_fd("21sh: history: -", YEL, 2);
...
int				ft_history_error(int mode, char *str, char c)
{
	if (mode == 1 || mode == 6)
	{
		...
		ft_putendl_col_fd("21sh: history: Too many arguments", YEL, 2);
	else if (mode == 3 || mode == 5 || mode == 7)
		ft_history_error_2(mode, c);
	else if (mode == 4)
	{
		ft_putstr_col_fd("21sh: history: cannot use more than one of ", YEL, 2);
...
static char		**ft_filecontent_2(char **env, int i)
{
	...
	{
		tmp = ft_strjoinfree(tmp, ".21sh_history", 1);
		cont = ft_hist_getcontent(tmp);
		free(tmp);
		if (cont == NULL)
			return (NULL);
		else
